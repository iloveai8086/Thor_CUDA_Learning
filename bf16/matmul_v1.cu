...
constexpr int TB_SIZE = NUM_WARPS * WARP_SIZE;  // 4 * 32 = 128
...
void matmul_v1_kernel(
  ...
  // https://docs.nvidia.com/cuda/parallel-thread-execution/#tcgen05-memory-consistency-model-canonical-sync-patterns-non-pipelined-diff-thread
  ...
  for (int n = 0; n < BLOCK_N / 8; n++) {
    ...
    // nv_bfloat16 *out_ptr = C_ptr + off_m * N + off_n + tid * N + n * 8
...
void matmul_v1_launch(
  ...
  // Max shared memory per SM: 102400 bytes
  // Max shared memory per block: 49152 bytes
  // Max shared memory per block (opt-in): 101376 bytes

  // Device ID: 0
  // Device Name: NVIDIA Thor
  // Compute Capability: 11.0
  // Max shared memory per SM: 233472 bytes
  // Max shared memory per block: 49152 bytes
  // Max shared memory per block (opt-in): 232448 bytes
  // int dev = 0;
  // cudaGetDevice(&dev);
  ...
  if constexpr (TMAP_3D) {
    ...
    auto init_tmap_AB = [&](CUtensorMap *tmap, 
                            const nv_bfloat16 *ptr, 
                            uint64_t global_height, 
                            uint32_t shared_height) {
      constexpr uint32_t rank = 3;
      uint64_t globalDim[rank]       = {8, global_height, (uint64_t)K / 8};
      uint64_t globalStrides[rank-1] = {(uint64_t)K * sizeof(nv_bfloat16), 16};
      uint32_t boxDim[rank]          = {8, shared_height, (uint32_t)BLOCK_K / 8};
      uint32_t elementStrides[rank]  = {1, 1, 1};
  ...
  int grid = (M / BLOCK_M) * (N / BLOCK_N);       // 512 = (4096 / 128) * (4096 / 256)
  int size_AB = (BLOCK_M + BLOCK_N) * BLOCK_K;    // 98304 = (256 + 128) * 256
  int smem_size = size_AB * sizeof(nv_bfloat16);  // 98304 * 2 = 196608
  // printf("grid = %d\n", grid);
  // printf("size_AB = %d\n", size_AB);
  // printf("smem_size = %d\n", smem_size);
