...
__device__ inline
int mapa_shared_cluster(int cta_addr, int cta_rank) {
  int cluster_addr;
...
void matmul_v5_2_kernel(
  ...
  auto load = [&](int iter_k) {
    ...
    // both CTA ranks update the TMA mbarrier that lives on CTA0.
    const int local_mbar_addr = tma_mbar_addr + stage_id * 8;
    const int mbar_addr = mapa_shared_cluster(local_mbar_addr, 0);
...
void matmul_v5_2_launch(
  ...
  auto this_kernel = matmul_v5_2_kernel<BLOCK_N, BLOCK_K, CTA_GROUP, NUM_STAGES, DO_PROFILE>;
  if (smem_size > 48'000)
...
void matmul_v5_2(
  ...
  matmul_v5_2_launch<256, 64, 2, 7, false>(A_ptr, B_ptr, C_ptr, M, N, K, nullptr, 0);
}

void profile_matmul_v5_2(
  const nv_bfloat16 *A_ptr,
  const nv_bfloat16 *B_ptr,
        nv_bfloat16 *C_ptr,
  int M, int N, int K,
  ...
  // matmul_v5_2_launch<256, 64, 2, 7, true>(A_ptr, B_ptr, C_ptr, M, N, K, profiler_ptr, num_entries);
  // matmul_v5_2_launch<256, 64, 1, 7, true>(A_ptr, B_ptr, C_ptr, M, N, K, profiler_ptr, num_entries);
  // matmul_v5_2_launch<256, 64, 2, 7, false>(A_ptr, B_ptr, C_ptr, M, N, K, profiler_ptr, num_entries);
  // matmul_v5_2_launch<256, 64, 2, 4, false>(A_ptr, B_ptr, C_ptr, M, N, K, profiler_ptr, num_entries);
  // matmul_v5_2_launch<256, 128, 2, 4, false>(A_ptr, B_ptr, C_ptr, M, N, K, profiler_ptr, num_entries);
  // matmul_v5_2_launch<128, 64, 1, 4, false>(A_ptr, B_ptr, C_ptr, M, N, K, profiler_ptr, num_entries);
  // matmul_v5_2_launch<256, 128, 2, 2, false>(A_ptr, B_ptr, C_ptr, M, N, K, profiler_ptr, num_entries);
  // matmul_v5_2_launch<256, 64, 4, 7, true>(A_ptr, B_ptr, C_ptr, M, N, K, profiler_ptr, num_entries);
}
