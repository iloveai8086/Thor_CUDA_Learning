...
namespace matmul_fp8_v6_detail {
...
void check_cu_v6(CUresult result, const char* expr) {
...
#define MATMUL_FP8_V6_CU_CHECK(expr) check_cu_v6((expr), #expr)
...
CUtensorMap make_tma_fp8_3d_128b_desc(
  ...
  MATMUL_FP8_V6_CU_CHECK(cuTensorMapEncodeTiled(
      &tensor_map,
...
void launch_matmul_fp8_v6(
  ...
  auto kernel = matmul_fp8_v6_kernel<BLOCK_N, BLOCK_K, NUM_STAGES>;
  if (smem_size > 48'000)
...
template <int BLOCK_N, int BLOCK_K, int NUM_STAGES>
static at::Tensor matmul_fp8_v6_impl(
    const at::Tensor& A,
    const at::Tensor& B,
    const char* op_name) {
  using namespace matmul_fp8_v6_detail;
  ...
  TORCH_CHECK(m % kBlockM == 0 && n % BLOCK_N == 0 && k % BLOCK_K == 0,
              op_name, " requires M%128==0, N%BLOCK_N==0, K%BLOCK_K==0");
  ...
  launch_matmul_fp8_v6<BLOCK_N, BLOCK_K, NUM_STAGES>(
      a_fp8, b_nt_fp8, C,
