...
constexpr int NUM_WARPS_V11 = 6;
constexpr int TB_SIZE_V11 = NUM_WARPS_V11 * WARP_SIZE;

constexpr int BLOCK_M_V11 = 128;
constexpr int MMA_K_V11 = 16;
constexpr int NUM_CLC_STAGES_V11 = 1;
constexpr int CTA_WORK_SYNC_BARRIER_V11 = 7;

__device__ inline void named_barrier_sync_v11(int barrier_id, int num_threads) {
  asm volatile("bar.sync %0, %1;" :: "r"(barrier_id), "r"(num_threads) : "memory");
}

template <int CTA_GROUP>
using ClcClusterShapeV11 = cute::Shape<cute::Int<CTA_GROUP>, cute::_1, cute::_1>;

template <int CTA_GROUP>
using ClcPipelineV11 = cutlass::PipelineCLCFetchAsync<NUM_CLC_STAGES_V11, ClcClusterShapeV11<CTA_GROUP>>;

template <int CTA_GROUP>
using ClcSchedulerV11 = cutlass::gemm::kernel::detail::PersistentTileSchedulerSm100<ClcClusterShapeV11<CTA_GROUP>, NUM_CLC_STAGES_V11>;

template <int CTA_GROUP>
using ClcResponseV11 = typename ClcSchedulerV11<CTA_GROUP>::CLCResponse;

template <int CTA_GROUP>
struct ClcSharedStorageV11 {
  alignas(16) ClcResponseV11<CTA_GROUP> clc_response[NUM_CLC_STAGES_V11];
  alignas(16) typename ClcPipelineV11<CTA_GROUP>::SharedStorage pipeline_storage;
};

template <int CTA_GROUP>
static __device__ inline void issue_clc_query_v11(
  typename ClcPipelineV11<CTA_GROUP>::PipelineState state,
  uint32_t mbarrier_addr,
  ClcResponseV11<CTA_GROUP> *clc_response_ptr
) {
...
__device__ inline bool decode_clc_response_v11(uint32_t result_addr, int &next_cluster_bid, int cluster_dim_x) {
...
__launch_bounds__(TB_SIZE_V11)
void matmul_v11_kernel(
  ...
  const int grid_m = M / BLOCK_M_V11;
  ...
  constexpr int A_size = BLOCK_M_V11 * BLOCK_K * sizeof(nv_bfloat16);
  ...
  __shared__ __align__(16) char clc_shared_bytes[sizeof(ClcSharedStorageV11<CTA_GROUP>)];
  auto &clc_shared = *reinterpret_cast<ClcSharedStorageV11<CTA_GROUP> *>(clc_shared_bytes);
  ...
  using ThisClcPipeline = ClcPipelineV11<CTA_GROUP>;
  using ThisClcPipelineState = typename ThisClcPipeline::PipelineState;

  const bool is_producer_warp = (cta_rank == 0 && warp_id == 0);
  const bool uses_clc_consumer = (warp_id == 0);
  const int cta_active_threads = WARP_SIZE * (cta_rank == 0 ? NUM_WARPS_V11 : (NUM_WARPS_V11 - 1));
  ...
  clc_params.consumer_arv_count = WARP_SIZE * CTA_GROUP;
  clc_params.initializing_warp = 0;
  ThisClcPipeline clc_pipeline(clc_shared.pipeline_storage, clc_params, ClcClusterShapeV11<CTA_GROUP>{});
  ...
  constexpr int MMA_M = BLOCK_M_V11 * CTA_GROUP;
  ...
  auto load = [&](int tma_stage, int mma_phase, int iter_k, int bid_m, int bid_n) {
    ...
    const int off_m = bid_m * BLOCK_M_V11;
  ...
  auto compute = [&](int tma_stage, int tma_phase, int mainloop_stage, int enable_input_d) {
    ...
    for (int k2 = 1; k2 < 64 / MMA_K_V11; k2++) {
    ...
    for (int k1 = 1; k1 < BLOCK_K / 64; k1++) {
      for (int k2 = 0; k2 < 64 / MMA_K_V11; k2++) {
        uint64_t a_desc = make_desc(A_smem + k1 * BLOCK_M_V11 * 128 + k2 * 32);
  ...
  auto epilogue = [&](int mainloop_stage, int bid_m, int bid_n) {
    ...
    for (int n = 0; n < BLOCK_N / 8; n++) {
      ...
      nv_bfloat16 *out_ptr = C_ptr + (bid_m * BLOCK_M_V11 + epilogue_tid) * N + (bid_n * BLOCK_N + n * 8);
  ...
  auto fetch_next_work = [&](ThisClcPipelineState &consumer_state, int current_cluster_bid) {
    ...
    if (lane_id == 0) {
      ...
      valid = static_cast<int>(decode_clc_response_v11(smem_addr, next_cluster_bid, CTA_GROUP));
    }
  ...
  auto sync_cta_work_state = [&]() {
    if (warp_id == 0 || warp_id >= 2 || (cta_rank == 0 && warp_id == 1)) {
      named_barrier_sync_v11(CTA_WORK_SYNC_BARRIER_V11, cta_active_threads);
    }
  };
  ...
  if (warp_id == 0) {
    ...
    while (has_work) {
      ...
      if (is_producer_warp) {
        ...
        if (elect_sync()) {
          issue_clc_query_v11<CTA_GROUP>(producer_state, clc_pipeline.producer_get_barrier(producer_state), clc_shared.clc_response);
        }
        ++producer_state;
      }
...
void matmul_v11_launch(
  ...
  init_tmap_AB(&A_tmap, A_ptr, M, BLOCK_M_V11);
  init_tmap_AB(&B_tmap, B_ptr, N, BLOCK_N / CTA_GROUP);

  const int num_tiles = (M / BLOCK_M_V11) * (N / BLOCK_N);
  const int grid = num_tiles * CTA_GROUP;
  const int size_AB = (BLOCK_M_V11 + BLOCK_N / CTA_GROUP) * BLOCK_K * 7;
  const int smem_size = size_AB * sizeof(nv_bfloat16);

  auto this_kernel = matmul_v11_kernel<BLOCK_N, BLOCK_K, CTA_GROUP>;
  if (smem_size > 48'000) {
  ...
  launch_config.blockDim = dim3(TB_SIZE_V11, 1, 1);
...
void matmul_v11(
  ...
  matmul_v11_launch<256, 64, 2>(A_ptr, B_ptr, C_ptr, M, N, K);
}