...
void matmul_v3_kernel(
  ...
  __shared__ uint64_t tma_mbars[NUM_STAGES];  // 2
  __shared__ uint64_t mma_mbars[1];           // 1
  ...
  const int num_iters = K / BLOCK_K;  // 4096 / 128 = 32

  for (int i = 0; i < NUM_STAGES - 1; i++)
    load(i);  // tma_3d_gmem2smem + mbarrier.arrive.expect_tx.release.cta.shared::cta.b64 _, [%0], %1;

  // 0-32-2+1=31，31 * 8 = 248
  #pragma unroll
  ...
  //   if (iter_k < num_iters - NUM_STAGES + 1)
  //     load(iter_k + NUM_STAGES - 1);
  //   compute(iter_k);
  //   // wait for MMA
  //   mbarrier_wait(mma_mbar_addr, mma_phase);
...
void matmul_v3_2_kernel(
  ...
  #pragma nv_diag_suppress static_var_with_dynamic_init
  __shared__ uint64_t tma_mbars[2];
  ...
  if (warp_id == 0 && elect_sync()) {
    ...
    mbarrier_init(tma_mbar_addr + 8, 1);
  ...
  const int tma_mbar_ping = tma_mbar_addr;
  const int tma_mbar_pong = tma_mbar_addr + 8;
  const int A_smem_ping = smem;
  const int B_smem_ping = A_smem_ping + A_size;
  const int A_smem_pong = smem + STAGE_SIZE;
  const int B_smem_pong = A_smem_pong + A_size;

  auto load_ping = [&](int iter_k) {
    if (warp_id == 0 && elect_sync()) {
      const int off_k = iter_k * BLOCK_K;
      tma_3d_gmem2smem(A_smem_ping, &A_tmap, 0, off_m, off_k / 64, tma_mbar_ping);
      tma_3d_gmem2smem(B_smem_ping, &B_tmap, 0, off_n, off_k / 64, tma_mbar_ping);
      asm volatile("mbarrier.arrive.expect_tx.release.cta.shared::cta.b64 _, [%0], %1;"
                  :: "r"(tma_mbar_ping), "r"(STAGE_SIZE) : "memory");
    }
  };

  auto load_pong = [&](int iter_k) {
    if (warp_id == 0 && elect_sync()) {
      const int off_k = iter_k * BLOCK_K;
      tma_3d_gmem2smem(A_smem_pong, &A_tmap, 0, off_m, off_k / 64, tma_mbar_pong);
      tma_3d_gmem2smem(B_smem_pong, &B_tmap, 0, off_n, off_k / 64, tma_mbar_pong);
      asm volatile("mbarrier.arrive.expect_tx.release.cta.shared::cta.b64 _, [%0], %1;"
                  :: "r"(tma_mbar_pong), "r"(STAGE_SIZE) : "memory");
    }
  };

  auto issue_mma = [&](int iter_k, int A_smem, int B_smem) {
    if (warp_id == 0 && elect_sync()) {
      auto make_desc = [](int addr) -> uint64_t {
        const int SBO = 8 * 128;
        return desc_encode(addr) | (desc_encode(SBO) << 32ULL) | (1ULL << 46ULL) | (2ULL << 61ULL);
  ...
  auto compute_ping = [&](int iter_k, int tma_phase) {
    mbarrier_wait(tma_mbar_ping, tma_phase);
    asm volatile("tcgen05.fence::after_thread_sync;");
    issue_mma(iter_k, A_smem_ping, B_smem_ping);
  };

  auto compute_pong = [&](int iter_k, int tma_phase) {
    mbarrier_wait(tma_mbar_pong, tma_phase);
    asm volatile("tcgen05.fence::after_thread_sync;");
    issue_mma(iter_k, A_smem_pong, B_smem_pong);
  };
  ...
  for (int iter_k = 0; iter_k + 1 < num_iters; iter_k += 2) {
    const int tma_phase = (iter_k >> 1) & 1;

    load_pong(iter_k + 1);
    compute_ping(iter_k, tma_phase);
    mbarrier_wait(mma_mbar_addr, iter_k & 1);

    if (iter_k + 2 < num_iters)
      load_ping(iter_k + 2);

    compute_pong(iter_k + 1, tma_phase);
    mbarrier_wait(mma_mbar_addr, (iter_k + 1) & 1);
  }

  if (num_iters & 1) {
    const int iter_k = num_iters - 1;
    compute_ping(iter_k, (iter_k >> 1) & 1);
    mbarrier_wait(mma_mbar_addr, iter_k & 1);
  }
...
void matmul_v3(
  ...
  matmul_v3_launch<256, 128, 2>(A_ptr, B_ptr, C_ptr, M, N, K);  // 3,45ms, 3.142     43.741
  // matmul_v3_launch<256, 64, 4>(A_ptr, B_ptr, C_ptr, M, N, K);   // 3.627     37.892
  // matmul_v3_launch<256, 256, 1>(A_ptr, B_ptr, C_ptr, M, N, K);  // 3.271     42.015
}

template <int BLOCK_N, int BLOCK_K>
void matmul_v3_2_launch(
  const nv_bfloat16 *A_ptr,
  const nv_bfloat16 *B_ptr,
        nv_bfloat16 *C_ptr,
  int M, int N, int K
  ...
  int grid = (M / BLOCK_M) * (N / BLOCK_N);
  int smem_size = 2 * (BLOCK_M + BLOCK_N) * BLOCK_K * sizeof(nv_bfloat16);

  auto this_kernel = matmul_v3_2_kernel<BLOCK_N, BLOCK_K>;
  if (smem_size > 48'000)
    cudaFuncSetAttribute(this_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size);
...
void matmul_v3_2(
  ...
  matmul_v3_2_launch<256, 128>(A_ptr, B_ptr, C_ptr, M, N, K);
}
