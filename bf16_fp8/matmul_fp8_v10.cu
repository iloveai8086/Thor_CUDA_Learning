...
namespace matmul_fp8_v10_detail {
...
void check_cu_v10(CUresult result, const char* expr) {
...
#define MATMUL_FP8_V10_CU_CHECK(expr) check_cu_v10((expr), #expr)
...
CUtensorMap make_tma_fp8_3d_128b_desc(
  ...
  MATMUL_FP8_V10_CU_CHECK(cuTensorMapEncodeTiled(
      &tensor_map,
...
__device__ inline void prefetch_tma_descriptor_v10(const void* tmap_ptr) {
  const uint64_t gmem_desc = reinterpret_cast<uint64_t>(tmap_ptr);
  asm volatile("prefetch.tensormap [%0];" :: "l"(gmem_desc) : "memory");
}

__device__ inline void tma_2d_smem2gmem_v10(const void* tmap_ptr, int smem_addr, int x, int y) {
  const uint64_t gmem_desc = reinterpret_cast<uint64_t>(tmap_ptr);
  asm volatile(
...
__device__ inline void tma_store_commit_v10() {
  asm volatile("cp.async.bulk.commit_group;");
}

template <int N>
__device__ inline void tma_store_wait_v10() {
  asm volatile("cp.async.bulk.wait_group.read %0;" :: "n"(N) : "memory");
}

__device__ inline void fence_async_shared_v10() {
...
__device__ inline int fp8_128b_swizzle_offset(int row, int col) {
  const int bank_group = col >> 4;   // 16 FP8 values == 16 bytes.
  const int inner = col & 15;
  return row * kTmaK + ((bank_group ^ (row & 7)) << 4) + inner;
}
...
void matmul_fp8_v10_kernel(
  ...
  if constexpr (USE_TMA_PREFETCH) {
    if (warp_id == 0 && elect_sync()) {
      prefetch_tma_descriptor_v10(&A_tmap);
      prefetch_tma_descriptor_v10(&B_tmap);
      if constexpr (USE_TMA_STORE)
        prefetch_tma_descriptor_v10(&C_tmap);
    }
  }
      ...
      for (int subtile = 0; subtile < NUM_SUBTILES; ++subtile) {
        ...
        if (c_store_count >= kTmaStoreStages) {
          if (warp_id == 2 && elect_sync())
            tma_store_wait_v10<kTmaStoreStages - 1>();
          epilogue_sync();
        }
        ...
        fence_async_shared_v10();
        epilogue_sync();

        if (warp_id == 2 && elect_sync()) {
          const int g_col = bid_n * BLOCK_N + subtile * kTmaStoreEpiN;
          const int g_row = bid_m * kBlockM;
          tma_2d_smem2gmem_v10(&C_tmap, c_smem_buf, g_col, g_row);
          tma_store_commit_v10();
        }
        ++c_store_count;
        epilogue_sync();
      }
...
void launch_matmul_fp8_v10(
  ...
  constexpr int smem_size =
      ...
      C_epi_smem_size;
  auto kernel = matmul_fp8_v10_kernel<
...
__device__ inline __nv_fp8x2_storage_t bf16_pair_to_fp8x2_e4m3(
    nv_bfloat16 lo,
    nv_bfloat16 hi) {
  __nv_bfloat162_raw raw;
  raw.x = __BFLOAT16_TO_CUS(lo);
  raw.y = __BFLOAT16_TO_CUS(hi);
  return __nv_cvt_bfloat16raw2_to_fp8x2(raw, __NV_SATFINITE, __NV_E4M3);
}
...
__global__ void cast_bf16_to_fp8_e4m3_kernel(
  ...
  if (idx + 1 < numel) {
    const __nv_fp8x2_storage_t packed = bf16_pair_to_fp8x2_e4m3(x[idx], x[idx + 1]);
    *reinterpret_cast<__nv_fp8x2_storage_t*>(y + idx) = packed;
  } else if (idx < numel) {
...
at::Tensor cast_bf16_to_fp8_e4m3_cuda(const at::Tensor& x) {
  ...
  const int64_t num_pairs = (numel + 1) / 2;
  const int blocks = static_cast<int>((num_pairs + threads - 1) / threads);
  auto stream = at::cuda::getCurrentCUDAStream().stream();
...
__global__ void transpose_cast_bf16_to_fp8_e4m3_kernel(
  ...
  __shared__ nv_bfloat16 tile[kTile][kTile + 1];

  const int x_col = blockIdx.x * kTile + threadIdx.x;
  const int x_row = blockIdx.y * kTile + threadIdx.y;

  #pragma unroll
  for (int j = 0; j < kTile; j += kBlockRows) {
    if (x_row + j < rows && x_col < cols)
      tile[threadIdx.y + j][threadIdx.x] = x[(x_row + j) * cols + x_col];
  }

  __syncthreads();

  const int y_col = blockIdx.y * kTile + threadIdx.x;
  const int y_row = blockIdx.x * kTile + threadIdx.y;

  #pragma unroll
  for (int j = 0; j < kTile; j += kBlockRows) {
    if (y_row + j < cols && y_col < rows) {
      const int y_idx = (y_row + j) * rows + y_col;
      if ((threadIdx.x & 1) == 0) {
        if (y_col + 1 < rows) {
          const __nv_fp8x2_storage_t packed = bf16_pair_to_fp8x2_e4m3(
              tile[threadIdx.x][threadIdx.y + j],
              tile[threadIdx.x + 1][threadIdx.y + j]);
          *reinterpret_cast<__nv_fp8x2_storage_t*>(y + y_idx) = packed;
        } else {
          const nv_bfloat16 v = tile[threadIdx.x][threadIdx.y + j];
          y[y_idx] = __nv_fp8_e4m3(__bfloat162float(v));
        }
      }
    }
  }
...
at::Tensor transpose_cast_bf16_to_fp8_e4m3_cuda(const at::Tensor& x) {
  TORCH_CHECK(x.is_cuda(), "transpose_cast_bf16_to_fp8_e4m3_cuda expects CUDA tensor");
  TORCH_CHECK(x.scalar_type() == at::kBFloat16,
              "transpose_cast_bf16_to_fp8_e4m3_cuda expects BF16 input");
  TORCH_CHECK(x.dim() == 2, "transpose_cast_bf16_to_fp8_e4m3_cuda expects 2D input");
  TORCH_CHECK(x.is_contiguous(), "transpose_cast_bf16_to_fp8_e4m3_cuda expects contiguous input");

  const int64_t rows = x.size(0);
  const int64_t cols = x.size(1);
  TORCH_CHECK(rows <= INT32_MAX && cols <= INT32_MAX,
              "transpose_cast_bf16_to_fp8_e4m3_cuda expects int32 dimensions");
  auto y = at::empty({cols, rows}, x.options().dtype(at::ScalarType::Float8_e4m3fn));
  if (rows == 0 || cols == 0)
    return y;

  constexpr int kTile = 32;
  constexpr int kBlockRows = 8;
  dim3 threads(kTile, kBlockRows);
  dim3 blocks((static_cast<unsigned int>(cols) + kTile - 1) / kTile,
              (static_cast<unsigned int>(rows) + kTile - 1) / kTile);
  auto stream = at::cuda::getCurrentCUDAStream().stream();
  transpose_cast_bf16_to_fp8_e4m3_kernel<<<blocks, threads, 0, stream>>>(
      reinterpret_cast<const nv_bfloat16*>(x.data_ptr()),
      reinterpret_cast<__nv_fp8_e4m3*>(y.data_ptr()),
...
static at::Tensor matmul_fp8_v10_impl(
    const at::Tensor& A,
    const at::Tensor& B,
    const char* op_name,
    int64_t* profiler_ptr = nullptr,
    int num_entries = 0) {
  using namespace matmul_fp8_v10_detail;
  ...
  auto b_bf16 = B.contiguous();
  auto a_fp8 = cast_bf16_to_fp8_e4m3_cuda(a_bf16);
  auto b_nt_fp8 = transpose_cast_bf16_to_fp8_e4m3_cuda(b_bf16);

  auto C = at::empty({m, n}, A.options());
  launch_matmul_fp8_v10<
...
at::Tensor matmul_fp8_v10_fused_ab(const at::Tensor& A, const at::Tensor& B) {
  return matmul_fp8_v10_impl<true, true, true>(A, B, "matmul_fp8_v10_fused_ab");
}
