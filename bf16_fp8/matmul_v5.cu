...
void matmul_v5_kernel(
  ...
  if constexpr (DO_PROFILE) if (elect_sync()) {
    ...
    profiler.start(ProfilerTag::Setup, make_profiler_meta(warp_id, cta_rank, -1, -1, bid, bid_m, bid_n));
  }
  ...
  auto load = [&](int iter_k) {
    ...
    if constexpr (DO_PROFILE) profiler.start(
      ProfilerTag::WaitMMA,
      make_profiler_meta(warp_id, cta_rank, stage_id, phase ^ 1, bid, bid_m, bid_n, iter_k)
    );
    ...
    if constexpr (DO_PROFILE) profiler.start(
      ...
      make_profiler_meta(warp_id, cta_rank, stage_id, phase, bid, bid_m, bid_n, iter_k)
    );
    ...
    // const int mbar_addr = (tma_mbar_addr + stage_id * 8);  // this is on CTA0
    ...
    // off_n -> off_n + cta_rank × (BLOCK_N / CTA_GROUP)
  ...
  // tma_3d_gmem2smem<CTA_GROUP>(..., mbar_addr);
  // tma_3d_gmem2smem<CTA_GROUP>(..., mbar_addr);
  // mbarrier_arrive_expect_tx(mbar_addr, A_size + B_size);
  ...
  auto compute = [&](int iter_k) {
    ...
    if constexpr (DO_PROFILE) profiler.start(
      ...
      make_profiler_meta(warp_id, cta_rank, stage_id, phase ^ (stage_id == NUM_STAGES - 1), bid, bid_m, bid_n, iter_k)
    );
    const int A_smem = smem + stage_id * (A_size + B_size);
    const int B_smem = A_smem + A_size;
  ...
  if constexpr (DO_PROFILE) if (elect_sync()) profiler.start(
    ...
    make_profiler_meta(warp_id, cta_rank, -1, 0, bid, bid_m, bid_n)
  );
  ...
  if constexpr (DO_PROFILE) if (elect_sync()) profiler.start(
    ...
    make_profiler_meta(warp_id, cta_rank, -1, -1, bid, bid_m, bid_n)
  );
...
// we specify the LOCAL A smem and B smem here. 
// the tensor cores hardware will know to fetch data from BOTH CTAs, 
// assuming we use the same offset across CTAs.
...
void matmul_v5_launch(
  ...
  // (128+256/2)×64=(128+128)×64=16384
  // 16384×2=32768 bytes=32 KiB
  // 32 KiB×7=224 KiB
...
void profile_matmul_v5(
  ...
  // matmul_v5_launch<256, 64, 1, 7, true>(A_ptr, B_ptr, C_ptr, M, N, K, profiler_ptr, num_entries);
  // matmul_v5_launch<256, 64, 2, 7, false>(A_ptr, B_ptr, C_ptr, M, N, K, profiler_ptr, num_entries);
  // matmul_v5_launch<256, 64, 2, 4, false>(A_ptr, B_ptr, C_ptr, M, N, K, profiler_ptr, num_entries);
  // matmul_v5_launch<256, 128, 2, 4, false>(A_ptr, B_ptr, C_ptr, M, N, K, profiler_ptr, num_entries);
  // matmul_v5_launch<128, 64, 1, 4, false>(A_ptr, B_ptr, C_ptr, M, N, K, profiler_ptr, num_entries);
  // matmul_v5_launch<256, 128, 2, 2, false>(A_ptr, B_ptr, C_ptr, M, N, K, profiler_ptr, num_entries);
  // matmul_v5_launch<256, 64, 4, 7, true>(A_ptr, B_ptr, C_ptr, M, N, K, profiler_ptr, num_entries);
}
